					...
					<div class="row mb-2">
						...
						<div class="col-sm-6">
							<ol class="breadcrumb float-sm-right">
								...
								<li class="breadcrumb-item active">Edit Comment Form</li>
							</ol>
						</div>
					</div>
				</div>
						...
						<div class="col-md-12">
							...
							<div class="card card-primary">
								...
								<form id="frmCategories"
									action="${pageContext.servletContext.contextPath}/admin/comment/update"
									method="post">
									<div class="card-body">
										<div class="form-group">
											<label for="name">Name</label>
												<input type="hidden" id="id_categories"
												name="id_comment" value="${commentEdit.id_comment}">
												<input type="text" class="form-control" id="comment" name="comment"
													placeholder="Enter name" maxlength="20"
														value="${commentEdit.comment}" disabled>
												<span id="nameType-error" class="error invalid-feedback"></span>
										</div>
										<div class="form-group">
											<label>Status</label> <select class="form-control"
												id="status_comment" name="status_comment" required="required">
												<option value="-1" selected="selected">Select
													status</option>
													<c:choose>
														<c:when test="${commentEdit.status_comment == 1}">												
	...
	<script>
		$('#btnType').on('click', function() {
			...
			if (name.val().length === 0) {
				hasSubmit.push(1);
				$('#name_comment').addClass('form-control is-invalid');
				$('#name_comment-error').html('Please enter name!');
			} else {
				$('#name_comment').removeClass('form-control is-invalid');
				$('#name_comment').addClass('form-control is-valid');
				$('#name_comment-error').html('');
			}
			if (status.val() === '-1') {
				hasSubmit.push(1);
				$('#status_comment').addClass('form-control is-invalid');
				$('#status_comment-error').html('Please choose category!');
			} else {
				$('#status_comment').removeClass('form-control is-invalid');
				$('#status_comment').addClass('form-control is-valid');
				$('#status_comment-error').html('');
			}
